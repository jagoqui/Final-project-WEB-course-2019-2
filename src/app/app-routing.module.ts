...
const routes: Routes = [
  ...
  {path: 'editor', loadChildren: () => import('./editor/editor.module').then((m) => m.EditorModule), canActivate: [CanEditGuard]},
  {
    ...
    canActivate: [CanAdminGuard, CanSuscriptorGuard]
  }, // TODO: Agregar multiple guards
  ...
  {
    path: 'store-items',
    loadChildren: () => import('./admin/store-items/store-items.module').then(m => m.StoreItemsModule),
